...
- (void)notifyDelegateWebSocketDidOpen:(PSWebSocket *)webSocket {
    [self executeDelegate:^{
        [self.delegate server:self webSocketDidOpen:webSocket];
    }];
}
- (void)notifyDelegateWebSocket:(PSWebSocket *)webSocket didReceiveMessage:(id)message {
    [self executeDelegate:^{
        [self.delegate server:self webSocket:webSocket didReceiveMessage:message];
    }];
}

- (void)notifyDelegateWebSocket:(PSWebSocket *)webSocket didFailWithError:(NSError *)error {
    [self executeDelegate:^{
        [self.delegate server:self webSocket:webSocket didFailWithError:error];
    }];
}
- (void)notifyDelegateWebSocket:(PSWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
    [self executeDelegate:^{
        [self.delegate server:self webSocket:webSocket didCloseWithCode:code reason:reason wasClean:wasClean];
    }];
}
- (void)notifyDelegateWebSocketDidFlushInput:(PSWebSocket *)webSocket {
    [self executeDelegate:^{
        if ([self.delegate respondsToSelector: @selector(server:webSocketDidFlushInput:)]) {
            [self.delegate server:self webSocketDidFlushInput:webSocket];
        };
    }];
}
- (void)notifyDelegateWebSocketDidFlushOutput:(PSWebSocket *)webSocket {
    [self executeDelegate:^{
        if ([self.delegate respondsToSelector: @selector(server:webSocketDidFlushOutput:)]) {
            [self.delegate server:self webSocketDidFlushOutput:webSocket];
        }
    }];
}
- (BOOL)askDelegateShouldAcceptConnection:(PSWebSocketServerConnection *)connection
    ...
    [self executeDelegateAndWait:^{
        if([self.delegate respondsToSelector:@selector(server:acceptWebSocketWithRequest:address:trust:response:)]) {
            ...
            accept = [self.delegate server:self
        ...
        } else if([self.delegate respondsToSelector:@selector(server:acceptWebSocketWithRequest:)]) {
            accept = [self.delegate server:self acceptWebSocketWithRequest:request];
        } else {
            accept = YES;
        }
    }];
